...
(function () {
  ...
  exports.getInstance = function getInstance(mode) {
    // Silently coerce unknown modes to the default so callers never crash.
    const m = VALID_PRECISIONS.includes(mode) ? mode : DEFAULT_PRECISION;

    if (!_cache.has(m)) {
      // "float" → native IEEE 754 number (math.js default).
      // "64"|"128"|"256" → BigNumber with that many significant decimal digits.
